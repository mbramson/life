defmodule Life.Resolver.UnrolledGridTest do
  ...
  describe "cell_below_left/4" do
    ...
    test "returns 1 if cell below left is 1" do
      assert cell_below_left({0, 0, 1, 0}, 1, 2) == 1
    end

    test "returns 0 if cell below left is 0" do
      assert cell_below_left({1, 1, 0, 1}, 1, 2) == 0
    end
  end

  describe "cell_below_right/4" do
    test "returns 0 for an empty list" do
      assert cell_below_right({}, 0, 0) == 0
    end

    test "returns 0 for single cell" do
      assert cell_below_right({1}, 0, 1) == 0
    end

    test "returns 0 for cell on right border" do
      assert cell_below_right({1, 1, 1, 1}, 1, 2) == 0
    end

    test "returns 0 for cell on lower border" do
      assert cell_below_right({1, 1, 1, 1}, 2, 2) == 0
    end

    test "returns 0 for cell on lower right border" do
      assert cell_below_right({1, 1, 1, 1}, 3, 2) == 0
    end

    test "returns 1 if cell below right is 1" do
      assert cell_below_right({0, 0, 0, 1}, 0, 2) == 1
    end

    test "returns 0 if cell below right is 0" do
      assert cell_below_right({1, 1, 1, 0}, 0, 2) == 0
    end
  end
end
