defmodule Life.Resolver do
  ...
  def step(grid) do
    ...
    next_step_unrolled_grid =
      calculate_neighbors(unrolled_grid_tuple, rows, columns)

    UnrolledGrid.roll(next_step_unrolled_grid, columns)
  end

  defp calculate_neighbors(unrolled_grid, rows, columns) do
    unrolled_grid
    |> reduce_grid(0, rows, columns, [])
    |> Enum.reverse
  end

  defp reduce_grid(unrolled_grid, current_cell, _, _, acc)
    when tuple_size(unrolled_grid) == current_cell do
    acc
  end
  defp reduce_grid(unrolled_grid, current_cell, rows, columns, acc) do
    ...
    reduce_grid(unrolled_grid, current_cell + 1, rows, columns, [next_step_cell_value | acc])
  end

  defp rows_in(grid), do: length(grid)
